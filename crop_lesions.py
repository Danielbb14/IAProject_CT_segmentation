...
def crop_to_cube(target, comp_indices, target_shape, pad_value=None):
    ...
    if pad_value is None:
        cropped_volume = np.zeros(
            (desiredVoxelsX, desiredVoxelsY, desiredVoxelsZ), dtype=target.dtype
        )
    else:
...
def crop_lesions_to_cubes(
    ...
    for comp_id in range(1, num_components + 1):
        ...
        if is_binary:
            ...
            overlapping_labels = np.unique(labeled_target[(labeled_mask == comp_id)])
            ...
            img_crop = crop_to_cube(
                image, comp_indices, target_shape, pad_value=pad_value
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_side_mm", type=float, default=5, help="Size to crop")
    ...
    parser.add_argument(
        "--append_to_path", type=str, default="", help="Ending to add to file"
    )
    parser.add_argument(
        "--is_binary",
        type=bool,
